#@title GraphSAGE 64 channels 5 layers
import torch
...
# Define the GraphSAGE model with 5 layers
class GraphSAGE(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=5):
        super(GraphSAGE, self).__init__()
        self.convs = torch.nn.ModuleList()
        self.convs.append(SAGEConv(in_channels, hidden_channels))  # First layer
...
# Function to evaluate the model and calculate average metrics
def evaluate(model, data):
    ...
    with torch.no_grad():
        ...
        prob = out.exp().cpu().numpy()  # Convert log_softmax output to probability for log_loss
...
def pretrain_citeseer(model, optimizer, data, epochs=200):
    ...
    best_acc = 0.0
    total_acc = 0.0  # Track total accuracy for average
    for epoch in tqdm(range(epochs)):
        loss = train(model, optimizer, data, criterion)
        acc = evaluate(model, data)[0]
        total_acc += acc  # Accumulate accuracy for average calculation

        if acc > best_acc:
            best_acc = acc
            torch.save(model.state_dict(), 'sage_citeseer.pth')  # Save the pretrained model

    avg_acc = total_acc / epochs  # Calculate average accuracy
    print(f'Average Pretrained Accuracy on Citeseer: {avg_acc:.4f}')
    return best_acc, avg_acc

# Function to initialize the model
def initialize_model(in_channels, out_channels, hidden_channels=64, num_layers=5):
    model = GraphSAGE(in_channels=in_channels, hidden_channels=hidden_channels, out_channels=out_channels, num_layers=num_layers).to(device)
    return model
...
def finetune_cora(model, data, epochs=200):
    ...
    print(f'Final Metrics - Accuracy: {acc:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1 Score: {f1:.4f}, LogLoss: {logloss:.4f}')
    print(f'Average Metrics over {epochs} epochs: Accuracy: {avg_acc:.4f}, Precision: {avg_precision:.4f}, Recall: {avg_recall:.4f}, F1 Score: {avg_f1:.4f}, LogLoss: {avg_logloss:.4f}')
...
load_pretrained_weights(model_cora, 'sage_citeseer.pth')
